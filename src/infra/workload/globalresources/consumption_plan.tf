resource "azurerm_consumption_budget_resource_group" "global_rg_budget" {
  name       = "${azurerm_resource_group.global.name}_budget"
  amount     = 100
  ...
  resource_group_id = azurerm_resource_group.global.id
  notification {
    threshold      = 80                     // 0-1000
